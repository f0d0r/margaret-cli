...
func requireDatabaseFile(path string) error {
	...
	if os.IsNotExist(err) {
		return fmt.Errorf("database %q does not exist: run \"margaret scan <path>\" first", path)
	}
...
func requireScannedData(ctx context.Context, q *db.Queries, path string) error {
	...
	if n == 0 {
		return fmt.Errorf("database %q holds no scanned data: run \"margaret scan <path>\" first", path)
	}
	return nil
}
